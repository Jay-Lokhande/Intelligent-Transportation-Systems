...
\[
\mathbf{w}(e) = (t_e,\; s_e,\; c_e)
\]

interpreted as **travel time** (seconds), **safety cost**, and **surveillance cost** (unitless, higher = worse), all **to be minimized**.

Parallel edges \(u \to v\) are allowed (multiple `Edge` objects in the adjacency list). They are distinguished by their **full** cost vector when relaxed, and by storing the **actual edge vector** used on the back-pointer (Section 4).

### 1.2 Path cost

For a path \(p = (v_0,\ldots,v_k)\) with edges \(e_i: v_{i-1}\to v_i\),

\[
\mathbf{C}(p) = \sum_{i=1}^{k} \mathbf{w}(e_i)
\]
...
For two cost vectors \(\mathbf{a}, \mathbf{b} \in \mathbb{R}^3_{\ge 0}\):

- \(\mathbf{a}\) **dominates** \(\mathbf{b}\) (strict Pareto dominance for minimization) iff  
  \(a_j \le b_j\) for all \(j \in \{1,2,3\}\) and \(a_j < b_j\) for **at least one** \(j\).

A path is **Pareto-optimal** (w.r.t. all \(s\)–\(t\) paths) if **no** other \(s\)–\(t\) path has a cost vector that dominates its cost vector.

**Goal:** enumerate **one** \(s\)–\(t\) path per **nondominated** cost vector at \(t\) (the Pareto set in objective space; multiple paths with the **same** \(\mathbf{C}\) are deduplicated by `insert_nondominated` when vectors are exactly equal).
...
| Structure | Role |
|-----------|------|
| `labels[v]` | `list` of nondominated **partial** cost vectors \(\mathbf{c}\) at node \(v\) |
...
\[
\text{key}(v,\mathbf{c}) = c_1 + c_2 + c_3
\]

i.e. `_scalar_key(c)`. This **does not** define a new objective; it only **orders** which partial label to expand next. Stale or dominated entries are skipped (Section 3.3).
...
- **Pareto set at \(t\):** Any \(s\)–\(t\) path whose cost is **dominated** by another \(s\)–\(t\) path would induce a dominated label at \(t\); such labels are never kept. Conversely, under exact search (no caps), standard label-setting arguments for additive vector costs on finite graphs ensure that **nondominated** labels at \(t\) correspond to **Pareto-optimal** paths (up to tie-breaking among paths with **identical** \(\mathbf{C}\)).
...
Back-pointer for a **successful** label \((v, \mathbf{c}_{\text{new}})\):

\[
\text{back}[(v, \mathbf{c}_{\text{new}})] = (u,\; \mathbf{c}_u,\; \mathbf{w}_{uv})
\]

where \(\mathbf{c}_{\text{new}} = \mathbf{c}_u + \mathbf{w}_{uv}\) for the **chosen** parallel edge.

**Reconstruction:** start at \((t, \mathbf{c}_t)\), follow `back` until \((s, \mathbf{0})\), collecting node sequence and edge vectors; reverse both lists.

The returned triple per solution is:

\[
(\mathbf{C}_{\text{path}},\; [v_0,\ldots,v_k],\; [\mathbf{w}_1,\ldots,\mathbf{w}_k])
\]
...
After a successful `insert_nondominated`, if `|labels[v]| > N`, the list is **sorted lexicographically** by \((c_1,c_2,c_3)\) and **truncated** to the first \(N\) vectors.
...
- `objective = 0` → minimize \(\sum t_e\)
- `objective = 1` → minimize \(\sum s_e\)
- `objective = 2` → minimize \(\sum c_e\)

**Algorithm:** Classic Dijkstra:

- `dist[v]` = best scalar distance found so far.
- Relax each edge with weight \(w = \mathbf{w}(e)[\text{objective}]\).
...
- **Time:** Roughly \(O(|E| \cdot L^2)\) in the worst case for naive dominance checks per insertion (here each insert scans `labels[v]`). With caps, \(L \le N\).
- **Space:** \(O(|V| \cdot L)\) for label lists plus heap and back-pointers.
...
**This codebase does not implement Multi-Objective A\*.** There is **no** separate admissible heuristic \(h(v)\) subtracted or added in an \(f = g+h\) sense for each objective. The heap key is only a **scheduling** priority on the **current partial cost** \(\mathbf{c}\).

A natural extension (future work) would attach a vector lower bound \(\mathbf{h}(v)\) toward \(t\) and define consistent rules for expanding labels while preserving Pareto optimality (e.g. NAMOA\*-style algorithms).
...
For reproducible experiments and CLI flags, see `REPORT.md` §7.1 and `README.md`.
